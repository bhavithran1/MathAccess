...
function Header({ route, onNavigate }) {
  const activeProgram = programs.find((program) => program.route === route);
        ...
        <nav className="nav-links" aria-label="Main navigation">
          ...
          {programs.map((program) => (
            <RouteLink
              ...
              className={route === program.route ? "active" : ""}
            >
              {program.shortTitle}
            </RouteLink>
          ))}
    ...
    <section className="ticker-band" aria-label="MathAccess program signal">
      <div className="ticker-track">
        {items.map((program, index) => (
          <span className="ticker-item" key={`${program.id}-${index}`}>
      ...
      <section className="section" id="events">
        <div className="container">
          ...
          <div className="event-grid">
            {orgEvents.map(([title, body], index) => (
              <article className="event-card" key={title}>
                <span className="card-code">EVENT / {String(index + 1).padStart(2, "0")}</span>
                <h3>{title}</h3>
                <p>{body}</p>
              </article>
            ))}
  ...
  return (
    <>
      ...
      {program.sections.map((section, sectionIndex) => (
        <section className={`section ${sectionIndex % 2 ? "alt" : ""}`} key={section.kicker}>
          <div className="container">
            ...
            <div className="insight-grid">
              {section.items.map(([title, body], index) => (
                <article className="insight-card" key={title}>
                  <span className="card-code">{String(index + 1).padStart(2, "0")}</span>
                  <h3>{title}</h3>
                  <p>{body}</p>
                </article>
              ))}
    ...
    <footer className="site-footer">
      <div className="container footer-inner">
        ...
        <nav className="footer-links" aria-label="Footer links">
          {programs.map((program) => (
            <RouteLink key={program.id} to={program.route} onNavigate={onNavigate}>
              {program.shortTitle}
            </RouteLink>
          ))}
        </nav>
...
function App() {
  ...
  const currentProgram = useMemo(() => programs.find((program) => program.route === route), [route]);
